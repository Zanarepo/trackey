...
function DynamicProducts() {
  ...
  const manualInputRef = useRef(null); // Ref for manual input field
  ...
  // Auto-focus manual input when scanner modal opens or after a scan
  useEffect(() => {
  ...
  // Process scanned barcode (shared logic for webcam, external, manual)
  const processScannedBarcode = useCallback((scannedCode) => {
    ...
    if (scannerTarget) {
      ...
      if (modal === 'add') {
        ...
        arr[deviceIndex] = trimmedCode;
        arr.push(''); // Auto-add new device ID field
        setEditForm((prev) => ({ ...prev, deviceIds: arr }));
        newDeviceIndex = arr.length - 1;
      }

      // Update scannerTarget for the new device ID field
      setScannerTarget({
        modal,
        productIndex,
  ...
  useEffect(() => {
    ...
    const handleKeypress = (e) => {
      ...
      // Scanners send keys rapidly (<50ms); ignore slow typing
      if (timeDiff > 50 && scannerBuffer) {
        setScannerBuffer(''); // Reset buffer if too slow (likely human typing)
      }

      if (e.key === 'Enter' && scannerBuffer) {
        // Scanner sent Enter key, process the buffered input
  ...
  videoConstraints: {
    ...
    const startScanner = async (attempt = 1, maxAttempts = 5) => {
      if (!videoRef.current || !scannerDivRef.current) {
...
await html5QrCodeRef.current.start(
  config.videoConstraints, // Use config.videoConstraints directly
  config,
  onScanSuccess,
  onScanFailure
);
  ...
  }, [showScanner, scannerTarget, externalScannerMode]);
  ...
  const createProducts = async (e) => {
    ...
    try {
      ...
      const existingIds = existingProducts
        ...
        .filter((id) => id);
      const duplicates = allNewIds.filter((id) => existingIds.some((eId) => eId.toLowerCase() === id.toLowerCase()));
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            {addForm.map((p, pi) => (
              <div
                key={pi}
                className="border border-gray-200 dark:border-gray-700 p-4 rounded-lg relative shadow-sm bg-gray-50 dark:bg-gray-800"
              >
                {addForm.length > 1 && (
                  <button
                    type="button"
